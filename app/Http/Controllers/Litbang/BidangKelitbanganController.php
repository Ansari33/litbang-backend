...
class BidangKelitbanganController extends APIController
{
    private $BidangKelitbanganRepository;
    private $KelitbanganRepository;

    public function initialize()
    {
        $this->BidangKelitbanganRepository = \App::make('\App\Repositories\Contracts\Litbang\BidangKelitbanganInterface');
        $this->KelitbanganRepository = \App::make('\App\Repositories\Contracts\Litbang\KelitbanganInterface');
    }

    public function list(Request $request)
    {
        $result = $this->BidangKelitbanganRepository->with([
    ...
    public function listWithDatatable(Request $request)
    {
        ...
        return $datatable = datatables()->of($this->BidangKelitbanganRepository
            ->relation($relations)
            ->get())
                          ...
                          <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                              <ul class="navi flex-column navi-hover py-2">
                                  <li class="navi-item" onclick="'.$btn_edit.'">
                                          <a href="/kelitbangan-edit-bidang/'.$data['id'].'" target="_blank" class="navi-link">
...
//            })
    ...
    {
        $result = $this->BidangKelitbanganRepository->with([])->find($request->id);
        if ($result) {
            return $this->respond($result);
        } else {
    ...
    public function create(Request $request)
    {
        ...
        $validator = $this->BidangKelitbanganRepository->validate($request);
        if ($validator->fails()) {
            return $this->respondWithValidationErrors($validator->errors()->all(), MessageConstant::VALIDATION_FAILED_MSG);
        } else {
            DB::beginTransaction();
            $result = $this->BidangKelitbanganRepository->create(
                [
                    'nama' =>  $request->nama,
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            DB::beginTransaction();
            $result = $this->BidangKelitbanganRepository
                ->where('id',$request->id)
                ->update(
                    [
                        'nama' =>  $request->nama,
