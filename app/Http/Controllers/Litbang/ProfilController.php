...
class ProfilController extends APIController
{
    ...
    {
        $this->profilRepository = \App::make('\App\Repositories\Contracts\Litbang\ProfilInterface');
    }
    ...
    public function listWithDatatable(Request $request)
    {
        ...
        return $datatable = datatables()->of($this->profilRepository
            ->relation($relations)
            ->get())
...
//            })
    ...
    {
        $result = $this->profilRepository->with([])->find($request->id);
        if ($result) {
            return $this->respond($result);
        } else {
...
//                }
    ...
    public function update(Request $request)
    {
        ...
        $validator = $this->profilRepository->validateUpdate($request);
        if ($validator->fails()) {
            return $this->respondWithValidationErrors($validator->errors()->all(), MessageConstant::VALIDATION_FAILED_MSG);
        } else {
            DB::beginTransaction();
            ...
            if ($result) {
                DB::commit();
                return $this->respondCreated($result, 'Profil Berhasil Diperbaharui!');
            } else {
                DB::rollBack();
    ...
    {
        $result = $this->profilRepository->delete($request->id);
        if ($result) {
